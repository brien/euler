...
int eight()
{
	...
	for( int i = 0; i < p.length() - 12; i++)
	{
		...
		atoi(p.substr(i+5,1).c_str()) *
		atoi(p.substr(i+6,1).c_str()) *
		atoi(p.substr(i+7,1).c_str()) *
		atoi(p.substr(i+8,1).c_str()) *
		atoi(p.substr(i+9,1).c_str()) *
		atoi(p.substr(i+10,1).c_str()) *
		atoi(p.substr(i+11,1).c_str()) *
		atoi(p.substr(i+12,1).c_str()) ;
		if( num > largest )
		{
			largest = num;
		}
